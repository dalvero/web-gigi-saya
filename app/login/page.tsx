...
export default function LoginPage() {
  ...
  // MENGAMBIL QUERY PARAMTER ROLE (DEFAULT : STUDENT)
   ...
   * handleLogin()
   * ----------------------------
   * Menangani proses login berdasarkan role.
   */
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // MENGHILANGKAN FOKUS AGAR KEYBOARD MENUTUP
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* TOMBOL TOGGLE KE LOGIN GURU */}
        <p className="text-sm text-center mb-4 text-gray-600">
          {isTeacher ? "Masuk sebagai Siswa?" : "Masuk sebagai Guru?"}{" "}
          <button
            type="button"
            onClick={() => router.push(isTeacher ? "/login" : "/login?role=teacher")}
