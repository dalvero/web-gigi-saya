...
export const studentService = {
   ...
   * 
   * @returns {Promise<Students[]>} - Daftar semua siswa.   
   */
   ...
   */
  async add(student: Omit<Students, "created_at">): Promise<void> {
    const supabase = getSupabaseClient();
    if (!supabase) throw new Error("Supabase client not initialized.");
   ...
   */
  async update(id: string, updated: Partial<Students>): Promise<void> {
      const supabase = getSupabaseClient();
      if (!supabase) throw new Error("Supabase client not initialized.");
